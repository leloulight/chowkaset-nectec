...
class Phpgmaps
{
    ...
    public $apiKey = 'AIzaSyC2oYtXXDQIcvrBdC962Q51CZ4x_v3cARc';                        // If you've got an API key you can use it by passing this parameter. Setup an API key here: https://code.google.com/apis/console
    ...
    public $center = '';        // Sets the default center location (lat/long co-ordinate or address) of the map. If defaulting to the users location set to "auto"

    public $class = 'phpgmap';                        // A class name if wishing to style the map further through CSS. Can also be useful if wanting it to be responsive etc.
    public $cluster = false;                    // Whether to cluster markers
    public $clusterGridSize = 80;                        // The grid size of a cluster in pixels
    ...
    public $draggableCursor = 'default';                        // The name or url of the cursor to display on a draggable object
    ...
    public $language = 'th';                        // The map will by default load in the language of the browser. This can be overriden however here. For a full list of codes see https://spreadsheets.google.com/pub?key=p9pdwsai2hDMsLkXsoM05KQ&gid=1
    ...
    public $map_height = "100%";                    // The height of the map container. Any units (ie 'px') can be used. If no units are provided 'px' will be presumed
    ...
    public $onclick = 'drop_row();';                        // The JavaScript action to perform when the map is clicked
    public $ondblclick = '';                        // The JavaScript action to perform when the map is double-clicked
    public $ondrag = 'drop_row()';                        // The JavaScript action to perform while the map is dragged
    ...
    public $onrightclick = '
        ...
        right_click_tooltip(lat,lng);
        var myLatLng = {lat: lat, lng: lng};
        var size = new google.maps.Size(33, 33);
        var imagess = base_url+"assets/img/map-pin.png";
        var image = {
            url: imagess,
            size: size,
            scaledSize: new google.maps.Size(30, 30),
            origin: new google.maps.Point(0,0),
            anchor: new google.maps.Point(17, 26)
        };
    ...
    ';                        // The JavaScript action to perform when the map is right-clicked
    ...
    public function add_marker($params = array())
    {
        ...
        $marker['onclick'] = 'place_kaset()';                                // JavaScript performed when a marker is clicked
        ...
        if ($marker['infowindow_content'] != "") {
            ...
            if ($marker['onclick'] != "") {
                $marker_output .= '
                google.maps.event.addListener(marker_'.$marker_id.', "click", function(event) {
                    map.setCenter(marker_'.$marker_id.'.getPosition());
                    place_kaset('.$marker['map_id'].');
                });
                ';//'.$marker['onclick'].'
            }
        }
    ...
    public function add_polygon($params = array())
    {
        ...
        $polygon['onmouseout'] = 'this.setOptions({
                        strokeColor: \'#ff0000\',
                        fillColor: \'#ff0000\'
                    });';                            // JavaScript performed when the mouse leaves the area of the polygon
        $polygon['onmouseover'] = 'this.setOptions({
                        strokeColor: \'#00ff00\',
                        fillColor: \'#00ff00\'
                    });';                            // JavaScript performed when the mouse enters the area of the polygon
        ...
        $polygon['zIndex'] = '9888';                                // The zIndex of the polygon. If two polygons overlap, the polygon with the higher zIndex will appear on top
        $polygon_output = '';
    ...
    public function add_ground_overlay($params = array())
    {
        ...
        $overlay['onclick'] = 'Hello';                                // JavaScript performed when a ground overlay is clicked
    ...
    public function create_map()
    {
        ...
        if ($this->maps_loaded == 0) {
            ...
            if ($this->cluster) {
                $this->output_js .= '
            <script type="text/javascript" src="'.URL::asset('assets/js/markerclusterer_compiled_phpgmap.js').'"></script>
            ';
            }
        }
        if ($this->jsfile == "") {
        ...
        $this->output_js_contents .= '
            var centreGot;
            var check_pin = 0; //เช๋คว่ามี pin ที่ map หรือไม่
        ...
        $this->output_html .= '<div id="'.$this->map_div_id.'" style="z-index = 1; height:'.$this->map_height.';"'.(($this->class != "") ? ' class="'.$this->class.'"' : '').'></div>';
